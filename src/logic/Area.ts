...
export default class Area {
  ...
  static generateAreas(maxServerLineOverride?: Record<string, number>, respawnTime?: number) {
    const areas = [] as Area[]
    for (const [name, properties] of Object.entries(this.defaultAreas)) {
      const servers = [] as Server[]
      for (const line of Enumerable.range(1, maxServerLineOverride?.[name] || properties.maxLine)) {
